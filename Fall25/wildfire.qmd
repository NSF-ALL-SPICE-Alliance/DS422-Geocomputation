...
```{r}
ggplot(data = number_of_fires_per_day, aes(x = ACQ_DATE, y = number_of_fires)) +
  geom_line()
```
...
```{r}
unzip("data/FirespotArea_LANDSAT_USA_contiguous_and_Hawaii_7d1.kmz", exdir = "data/fire_kml")

wildfire_area_data <- st_read(
  "data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1757638503534.kml",
...
```{r}
fire_sf <- st_as_sf(fire_csv_data, coords = c("longitude", "latitude"), crs = 4326)
```
