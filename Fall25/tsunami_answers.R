...
## Challenge 1 - Using the Tsunami_Evacuation_All_Zones.geojson (https://geoportal.hawaii.gov/datasets/437b77f132ed416294b45bde6aef23f4_11/explore?location=20.546870%2C-157.491600%2C7.83) can you tell me how many people are in Tsunami Safe Zones vs Tsunami Evacuation Zones vs Extreme Tsunami Evacuation Zones
...
normal_tsunami_zones <- evac_all_zones[(evac_all_zones$zone_type %in% "Tsunami Evacuation Zone"),]

normal_tsunami_zones <- normal_tsunami_zones |>
  select(evac_id, island, zone_type, zone_desc)


normal_tsunami_zones_intersections <- st_intersection(block_groups, normal_tsunami_zones)

normal_tsunami_zones_intersections <- normal_tsunami_zones_intersections |>
  mutate(overlap_area = st_area(geometry))
...
sum(normal_tsunami_zones_intersections$pop_in_zone)

######

safe_zones <- evac_all_zones[(evac_all_zones$zone_type %in% "Tsunami Safe Zone"),]

safe_zones <- safe_zones |>
  select(evac_id, island, zone_type, zone_desc)


safe_zones_intersections <- st_intersection(block_groups, safe_zones)

safe_zones_intersections <- safe_zones_intersections |>
  mutate(overlap_area = st_area(geometry))
...
sum(safe_zones_intersections$pop_in_zone)
