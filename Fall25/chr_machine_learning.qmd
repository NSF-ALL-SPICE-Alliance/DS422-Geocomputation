...
# Download U.S. county shapefile (simple features)
...
chr_map <- chr_data %>%
  select(fips, percent_fair_or_poor_health) %>%
  mutate(fips = str_pad(fips, width = 5, pad = "0"))  # ensure 5-digit FIPS codes

# Join health data with county geometry
map_data <- counties %>%
  left_join(chr_map, by = "fips")
...
ggplot(map_data) +
  ...
  labs(
    ...
    caption = "Source: County Health Rankings & Roadmaps (2025)"
  ) +
  theme_map() +
