...
```{r}
hi_households_internet <- get_acs(
  geography = "tract",
  variables = c("B28002_004", "B01003_001"),  # median income
  state = "HI",
  year = 2023,
  geometry = TRUE
)
...
```{r}
glimpse(hi_median_income)
```

```{r}
library(sf)

# --- 1) Pivot to wide & keep geometry once per tract ---
geom_lookup <- hi_households_internet %>%
  select(GEOID, geometry) %>%
  distinct()

wide <- hi_households_internet %>%
  st_drop_geometry() %>%
  filter(variable %in% c("B01003_001", "B28002_004")) %>%
  mutate(var = recode(variable,
                      "B01003_001" = "pop",
                      "B28002_004" = "internet_hh")) %>%
  select(GEOID, NAME, var, estimate) %>%
  pivot_wider(names_from = var, values_from = estimate)

hi_net_per_pop <- geom_lookup %>%
  left_join(wide, by = "GEOID") %>%
  st_as_sf() %>%
  st_make_valid()

# --- 2) Compute households-with-internet per 1,000 residents ---
hi_net_per_pop <- hi_net_per_pop %>%
  mutate(
    internet_per_1000 = ifelse(pop > 0, 1000 * internet_hh / pop, NA_real_)
  )
...
hi_map |>
  add_fill_layer(
    id           = "hi_net_per_pop",
    source       = hi_net_per_pop,
    fill_color   = continuous_scale$expression,
    fill_opacity = 0.5,
    tooltip      = "internet_per_1000"
  ) |>
  add_legend(
    legend_title = "Households w/ Internet per 1,000 Residents",
    values       = get_legend_labels(continuous_scale, digits = 1),
    colors       = get_legend_colors(continuous_scale),
    type         = "continuous"
  )
```
