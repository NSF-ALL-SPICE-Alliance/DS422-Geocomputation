...
2.  **Explore patterns** of SNAP participation across Hawaiʻi census
    tracts
...
Each variable code (like "B17001_002") refers to a specific data table
and line number from the Census Bureau. The naming convention is: -
**B** = Detailed table - **17001** = Table ID (in this case, poverty
data) - **002** = Specific line number (usually the numerator for a
percentage calculation)

For more info, check the [Census Bureau Data API
...
::: {.callout-note icon="✅"}
Census data loaded successfully! (Output suppressed to keep document
clean)
:::
...
we use 1 as the denominator instead. The `pmax()` function takes the
parallel maximum - comparing each value to 1 element-wise.
:::
...
**Hints & Resources:** - Use the `hi_acs` dataset (it already has
geometry)
...
# Remember to include title, subtitle, and source citation!
ggplot(hi_acs) +
...
**Hints & Resources:** - The `mapview()` function creates interactive
maps - Use `zcol = "snap_pct"` to color by SNAP participation - Set
`legend = TRUE` to show a legend - Use `layer.name` to label your
layer - The `at` parameter controls color breaks (try:
`seq(0, max(...), by = 5)`) - Use
`col.regions = viridis::viridis(10, direction = -1)` for colors
...
With `mapview`, you can: - Zoom and pan - Click on tracts to see exact
values - Toggle layers on/off - Switch between map types - Export the
map
...
Create a histogram showing the distribution of `snap_pct` across all
Hawaii census tracts.

**Hints & Resources:** - Use `ggplot()` with `geom_histogram()` -
Consider bin width - try `bins = 20` or `binwidth = 2` - Add meaningful
labels - What does the shape tell you? Is it normally distributed?
...
**Your Model Should:** - Use `snap_pct` as the target variable (left
side of `~`) - Use all other variables as predictors (`.` on right
side) - Have `ntree = 500` for stability - Be named `rf`

**Hints & Resources:** - Use the `randomForest()` function - Formula
syntax: `target ~ .` predicts target using all other columns -
Structure: `randomForest(formula, data = ..., ntree = 500)` - Print the
model to see performance metrics!
...
1.  **Mean of squared residuals (MSR)**: How accurate are the
    predictions on average?
2.  **% Var explained**: What percentage of SNAP variation does the
    model explain?
...
SHAP (SHapley Additive exPlanations) values explain *why* the model
...
1.  **Unify the model**: Convert the Random Forest into a format that
    SHAP can work with
2.  **Calculate SHAP values**: Compute explanations for a subset of
    observations (computing for all can be slow)
3.  **Interpret**: Use the results to explain individual predictions and
    global patterns
...
2.  **Calculating SHAP values** for the first 200 observations using
    `treeshap()`
...
-   Step 2: Use `treeshap(unified_object, data_subset, verbose = 0)`
    ...
    -   Second argument: subset of data - use `hi_acs_data_ml[1:200, ]`
        to get first 200 rows
...
your computer. The `verbose = 0` parameter keeps it quiet while it
works. You'll know it's done when the next code chunk starts executing!
:::
...
**Hints & Resources:** - Use
`plot_feature_importance(treeshap1, max_vars = 5)` - This uses the SHAP
values we just calculated - The plot shows which features have the
biggest impact on predictions - Longer bars = more important
...
Create feature dependence plots for the **top 5 most important
features** you identified in Challenge 5.

**What to Do:** 1. Look at your feature importance plot 2. Identify the
5 most important variables 3. Create a dependence plot for each one 4.
Use: `plot_feature_dependence(treeshap1, "variable_name")`

**Hints & Resources:** - The x-axis shows the actual variable values -
The y-axis shows SHAP values (how much it pushes predictions up/down) -
Red dots = push toward higher SNAP % - Blue dots = push toward lower
SNAP % - Look for patterns: linear? non-linear? threshold effects?
...
1.  **Direction**: Does higher variable value → higher or lower SNAP
    participation?
...
Example answer: \> "Poverty rate shows a strong positive relationship
with SNAP participation. As poverty increases, SHAP values increase
substantially, pushing predictions toward higher SNAP participation. The
...
Examine what drives predictions for **Census Tract 304.04 on Maui,
Hawaiʻi**.

**Hints & Resources:** - This tract is observation #154 in our data
(hint: `obs = 154`) - Use: `plot_contribution()` - **Red bars** =
pushing toward higher SNAP % - **Blue bars** = pushing toward lower SNAP
% - **Bar length** = strength of effect
:::
...
2.  **Pushers**: Which 2-3 variables push predictions toward HIGHER
    SNAP%?
3.  **Pullers**: Which 2-3 variables push predictions toward LOWER
    SNAP%?
4.  **Net Effect**: Is the final prediction above or below the baseline?
5.  **Story**: Write 2-3 sentences telling the "story" of this tract's
    SNAP participation
:::
...
1.  **Model Performance**: How well does your Random Forest predict SNAP
    participation? What might explain the remaining unexplained
    variation?

2.  **Key Drivers**: Based on feature importance and dependence plots,
    which 2-3 factors are most strongly associated with SNAP
    participation in Hawaiʻi?

3.  **Equity Implications**:

    -   What does the geographic distribution of SNAP participation tell
        you about food security disparities?
...
6.  **Machine Learning Interpretation**: How did SHAP values help you
    understand the Random Forest model compared to just looking at
    variable importance?

7.  **Next Steps**: If you were to continue this analysis, what would
    you do next?
:::
...
-   Create scatter plots showing relationships between top predictors
    and SNAP%
...
-   Build a **second** Random Forest with different parameters (e.g.,
    `mtry`, `nodesize`)
...
In RStudio, click the blue **"Render"** button at the top of the editor
pane.

Wait for the rendering to complete - you'll see the document appear in
the **Viewer pane** (bottom right).

::: {.callout-tip icon="⏱️"}
**Rendering typically takes 2-5 minutes depending on how fast your
computer is. Be patient!**
:::

### Step 2: Open in New Window

In the **Viewer pane**, click the **"Show in new window"** icon (it
looks like an arrow pointing to a box, usually in the top-right corner
of the viewer).
...
1.  In your browser, press **`Ctrl + P`** (Windows) or **`Cmd + P`**
    (Mac)
...
Your PDF will download to your Downloads folder (or wherever your
browser saves files).
...
-   ✅ Make sure your `.qmd` file renders **without errors** before
    saving as PDF
-   ✅ Double-check that all your challenge code is visible in the HTML
    output
-   ✅ Your PDF should include both your code AND the results/plots
-   ✅ Save the PDF file somewhere you can find it (Desktop or a course
    folder)
...
**"Render button is grayed out?"** - Make sure you have the `.qmd` file
open in the editor (not just viewed)

**"I don't see a Viewer pane?"** - Go to `View` → `Panes` →
`Show Viewer Pane`

**"Nothing appears after clicking Render?"** - Check the R console for
error messages - Make sure all your code runs without errors first - Try
re-rendering after fixing any issues
...
-   [Random Forests in
    R](https://cran.r-project.org/web/packages/randomForest/randomForest.pdf)
-   [mapview Package](https://r-spatial.github.io/mapview/)

------------------------------------------------------------------------

*Good luck! Remember: focus on understanding WHAT you're doing and WHY,
not just running code. Ask questions, explore the data, and enjoy the
detective work of data science! 🔍📊*
